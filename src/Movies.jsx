...
const Movies = () => {
    const { data: movies, error, isLoading } = useFetch('https://www.omdbapi.com/?s=batman&apikey=56acf636&type=movie');


    return ( 
        <div className="movies py-6 bg-gray-100 min-h-screen px-4 md:px-28 py-8 ">
            {error && <div className="text-red-500 text-center">{error}</div>}
            {isLoading && <div className="text-center text-lg">Loading...</div>}
            
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {movies && movies.map((item) => (  // ✅ Mapping through series.Search
